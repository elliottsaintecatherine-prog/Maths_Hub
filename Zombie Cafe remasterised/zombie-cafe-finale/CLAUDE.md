...
**GROUPE COURANT : 5a1→5a3 + 5b1→5b4** (Shop + placement — enchaîner dans la même conversation)
...
5. 4c1→4c3 (Fin raid) ✓ TERMINÉ
6. ✦ 5a1→5a3 + 5b1→5b4 (Shop + placement) ← COURANT
...
| 4c1 | Conditions victoire/défaite                          | [x]    |
| 4c2 | Popup résultat : victoire                            | [x]    |
| 4c3 | Popup défaite + cooldowns                            | [x]    |
...
**Groupe 4c (Fin raid) terminé.** Le groupe courant est désormais **5a1→5a3 + 5b1→5b4 (Shop + placement)**.

**Prompt 5a1 — Bouton Shop + panneau 520x400 (1h)**

Crée le bouton Shop dans le HUD bas et un panneau modal 520x400 avec 4 onglets (Cuisine, Salle, Déco, Tombstones) + bouton X de fermeture.

CONTENU :
- Bouton Shop (rectangle vert 100x32) dans HUD bottom, ouvre le panneau au clic.
- Panneau modal 520x400 centré, fond beige, bordure marron foncé, `setDepth(900)`.
- Header avec 4 onglets (boutons texte 100x32 chacun) : Cuisine / Salle / Déco / Tombstones. Onglet actif = surligné.
- Bouton X (rectangle rouge 28x28) en haut-droite, ferme le panneau.
- Stocker `this.shopPanel = []`, `this.shopActiveTab = 'cuisine'`.
- Pour ce prompt : seule la structure visible. Contenu des onglets vide (à venir en 5a2/5a3/5c1/5d1).

CONTRAINTES :
- AUCUN emoji.
- Constantes nouvelles : `COLOR_SHOP_BG = 0xf4e8c8`, `COLOR_SHOP_BORDER = 0x6b4423`, `COLOR_TAB_ACTIVE = 0xffcc33`, `SHOP_W = 520`, `SHOP_H = 400`.
- Le clic en dehors du panneau ne ferme PAS (seul le X ferme).
- Le panneau doit pouvoir être ré-ouvert/fermé plusieurs fois sans fuite (destroy avant rebuild).

VÉRIFICATION : test_5a1.py qui vérifie ouverture/fermeture, switch d'onglet actif, état initial.

À la fin : coche [x] 5a1 et copie le texte de 5a2 dans PROCHAINE ACTION.
